...
for n in range(0, N_levels):
    ...
    contour_vertices = contour_path[0].vertices  # (N, 2) array of (R, Z) points
    is_it_closed = is_closed(contour_vertices)
    if is_it_closed:
...
def contour_analysis(contour_points, contour_psi):
    ...
    ivp_sol = solve_ivp(rhs, t_span=(0, L), y0=[0, 1e-3], dense_output=True) 
    ...
    root, root_ind = array_zero_finder(l, gamma)
        
    return(gamma, displacement, root, root_ind)
...
for n in range(0, N_closed):
    gam, disp, rot, rot_ind = contour_analysis(contours[n], psi_levels[n])
    gammas.append(gam)
